...
bool CLeg::IsRangeOK( double price ) {
   ...
   // for Oscilator indicator range

   if ( mRangeValue <= 0 ) return true;
   if ( mRangeIndicatorObject == NULL ) return false;

   mRangeIndicatorObject.Refresh();
   double value = mRangeIndicatorObject.GetData( mRangeIndicatorBuffer, mRangeIndicatorIndex );
   if ( GT( mRangeValue, value ) ) return false;
   return true;
}
